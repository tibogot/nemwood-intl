...
export default function Navigation8() {
  ...
  const translatedNavItems = localizedNavItems.map((item) => ({
    ...item,
    translatedName: t(item.labelKey),
  }));
  ...
  useEffect(() => {
    ...
    // Use requestAnimationFrame to ensure DOM is ready and translations are loaded
    requestAnimationFrame(() => {
      setTimeout(createSplitTextInstances, 200); // Increased delay for translations
    });
  ...
  }, [fontsLoaded, language]); // Only depend on language, not the full array
  ...
  const openMenu = contextSafe(() => {
    ...
    // 4. Show menu items container with enhanced stagger
    tl.to(
    ...
    // 5. Animate individual letters with enhanced stagger
  ...
  const closeMenu = contextSafe(() => {
    ...
    // 4. Collapse navbar back to original size
    tl.to(
      menuRef.current,
      {
    ...
    // 5. Reset burger lines with bounce effect
    tl.to(
    ...
    // 6. Reset characters to initial state after closing
    tl.call(() => {
      splitTextInstances.current.forEach((split) => {
      ...
      >
        <div className="relative z-10 h-16 px-4 md:px-8">
          <div className="flex h-16 items-center justify-between">
            {/* Logo - Much Larger */}
            {pathname === getLocalizedRoute("", language) ? (
            ...
            {/* Right side - Language Switcher, Theme Toggle and Burger Menu */}
